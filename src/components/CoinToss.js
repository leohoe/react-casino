    ...
    render() {
        return (
            <div>
                ...
                <div id="controls">
                    <button disabled={this.isBetTooHigh()} onClick={() => this.tossCoin(0)}>Kopf</button>
                    <button disabled={this.isBetTooHigh()} onClick={() => this.tossCoin(1)}>Zahl</button>
                </div>

                {this.state.lastOutcome !== null ? (
                    <div>
                        <h3>
                            Letzter Wurf: {this.state.lastOutcome === 0 ? "Kopf" : "Zahl"}
                        </h3>                    

                        {this.state.lastTossWon ? (
                            <div className="win-indicator">
                                <span>Wurf gewonnen!</span><br/>
                                <span>+{this.state.lastBetAmount} Chips!</span>
                            </div>
                        ) : (
                            <div className="loss-indicator">
                                <span>Wurf verloren...</span><br/>
                                <span>-{this.state.lastBetAmount} Chips.</span>
                            </div>
                        )}
                    </div>
