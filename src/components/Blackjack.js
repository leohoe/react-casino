...
class Blackjack extends Component {
    ...
    getPlayerScoreDisplayClass = () => {
        return (
            this.state.winner === "player" ? "score-display-winner" : (
                this.state.winner === "dealer" ? "score-display-loser" : (
                    "score-display-tie"
                )
            )
        )
    ...
    getDealerScoreDisplayClass = () => {
        return (
            this.state.winner === "dealer" ? "score-display-winner" : (
                this.state.winner === "player" ? "score-display-loser" : (
                    "score-display-tie"
                )
            )
        )
    ...
    getHandValue(hand) {
        ...
        for (let card of hand) {
            switch(card.rank)
            {
                case 'B': case 'D': case 'K':
                    sum += 10; break;
                case 'A':
                    sum += 11; aces++; break;
                default:
                    sum += Number(card.rank); break;
            }
            ...
            <div>
                ...
                <div>
                    ...
                    {(this.state.roundEnding || !this.state.roundInProgress) && (
                        <form className="bet-form" onSubmit={()=> {return false;}}>
                            <label>Wette Eingeben:<br/>
                                <input 
                                    type="number" 
                                    name="bet" 
                                ...
                                /><br/>
                                {this.isBetTooHigh() ? <div className="bet-excessive-indicator">Wette zu hoch!</div> : <></>}
                            </label>
                            <div>
                            <button type="submit" disabled={this.isBetTooHigh()} onClick={this.startRound}>Neue Runde</button>
                            </div>
                        </form>
                    )}
                    ...
                    {this.state.roundInProgress && (
                        <div>
                            {!this.state.roundEnding && (<div id="controls">
                                <button onClick={this.hit}>Hit</button>
                                <button onClick={this.stand}>Stand</button>
                                <button onClick={this.double} disabled={this.props.chips < 2 * this.state.betAmount}>Double</button>
                            </div>)}
                                    ...
                                    <div>
                                        ...
                                        <div className="card-wrapper">  
                                            {this.isBusted(this.state.playerCards) && (
                                                <div id="player-bust" className="bust-indicator">BUSTED</div>
                                            )}

                                            {this.state.playerCards.map((card) => {
                                                return (
                                                    <div key={card.suit+card.rank} className={['♥','♦'].includes(card.suit) ? "red-text" : ""}>
                                                        {card.suit + " " + card.rank}
                                                    </div>
                                                )
                                            })}
                                        ...
                                        {this.isBlackjack(this.state.playerCards) && (
                                            <div id="player-blackjack" className="blackjack-indicator">BLACK<br/>JACK</div>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="card-wrapper">  
                                            {this.isBusted(this.state.dealerCards) && (
                                                <div id="dealer-bust" className="bust-indicator">BUSTED</div>
                                            )}
                                            ...
                                            {this.state.dealerCards.map((card, index) => {
                                                return (
                                                    <div 
                                                        ...
                                                        className={
                                                            this.isDealerCardHidden(index) ? "gray-text" : (
                                                                ['♥','♦'].includes(card.suit) ? "red-text" : ""
                                                            )
                                                        }
                                                    >
                                        ...
                                        {this.isBlackjack(this.state.dealerCards) && (
                                            <div id="dealer-blackjack" className="blackjack-indicator">BLACK<br/>JACK</div>
                                        )}
                                    </div>
                            ...
                            {this.state.roundEnding ? (
                                this.state.winner === "player" ? (
                                    <div className="win-indicator">Runde gewonnen!<br/>+{this.state.lastPayout} Chips!</div>
                                ) : (
                                    this.state.winner === "dealer" ? (
                                        <div className="loss-indicator">Runde veloren...<br/>{this.state.lastPayout} Chips.</div>
                                    ) : (
